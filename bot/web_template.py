...
web_template
============
HTML template for the DocBot Web UI.

Stored as a Python string constant so the server is self-contained —
no static file directory or template engine required.
The HTML, CSS, and JavaScript are all inlined in a single document.
"""
...
document.querySelectorAll('.eq').forEach(eq => {
  eq.addEventListener('click', () => { document.getElementById('input').value = eq.textContent; sendMessage(); });
});
...
async function sendMessage() {
  ...
  try {
    ...
    const data = await fetch('/ask', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({question:q,collection:col}) }).then(r => r.json());
    let srcHtml = data.source_files?.length ? `<div class="sources"><div class="label">Sources</div>${data.source_files.map(s=>`<div class="src">📄 ${escapeHtml(s)}</div>`).join('')}</div>` : '';
    const c = data.confidence || 'MEDIUM';
    thinking.innerHTML = `<div class="bubble"><div class="answer">${escapeHtml(data.answer)}</div>${srcHtml}<span class="confidence ${c}">${c} confidence</span></div>`;
  } catch(e) {
